...
export const registerFarmerAsAgent = async (req, res) => {
    try {
        ...
        // Get agent profile
        const agentProfile = await prisma.fieldAgent.findUnique({ where: { userId: req.user.id } })
        if (!agentProfile) {
        ...
        const farmer = await prisma.user.create({
            data: {
                ...
                region,
                role: 'FARMER',
                isVerified: true, // agent-registered farmers are pre-verified
...
export const requestAgent = async (req, res) => {
    try {
        ...
        if (farmer.agentId) {
            return res.status(400).json({ message: 'You already have an agent managing your account' })
        }
...
export const getAgentRequests = async (req, res) => {
    try {
        ...
        const requests = await prisma.agentRequest.findMany({
            where: { agentId: agentProfile.id },
            include: {
                farmer: { select: { id: true, fullName: true, email: true, region: true } }
            },
